                ...
                <div class="col-12 col-xl-8">
                    <div class="archive-posts-area bg-white p-30 mb-30 mt-30 bx-shadow">
                        @foreach ($berita_nasional as $n)
                            <div class="single-catagory-post d-flex flex-wrap">
                                <div class="post-thumbnail bg-img"
                                    style="background-image: url({{ url('storage/berita/' . $n->foto) }});">
                                </div>
                                <div class="post-content">
                                    <a href="{{ route('berita.show', $n->slug) }}"
                                        class="post-title">{{ $n->judul }}</a>
                                    <div class="post-meta-2">
                                        <a href="#"><i class="bi bi-calendar"></i>
                                            {{ date('d M Y', strtotime($n->tanggal)) }}</a>
                        ...
                        <div class="d-flex justify-center mb-30">
                            {{ $berita_nasional->links('pagination::bootstrap-4') }}
                        </div>
                    </div>
                </div>
