                        ...
                        @foreach ($berita_dinas as $d)
                            <div class="single-catagory-post d-flex flex-wrap">
                                <div class="post-thumbnail bg-img"
                                    style="background-image: url({{ url('storage/berita/' . $d->foto) }});">
                                </div>
                                <div class="post-content">
                                    <a href="{{ route('berita.show', $d->slug) }}"
                                        class="post-title">{{ $d->judul }}</a>
                                    <div class="post-meta-2">
                                        <a href="#"><i class="bi bi-calendar"></i>
                                            {{ date('d M Y', strtotime($d->tanggal)) }}</a>
                                        <a href="#"><i class="bi bi-clock"></i> {{ $d->jam }} WIB</a>
                                        <a href="#"><i class="bi bi-eye"></i> {{ $d->viewer }} x</a>
                                    </div>
                                    <p> {!! Str::limit($d->isi, $limit = 300, $end = '...') !!}</p>
                                </div>
                            </div>
                        @endforeach
                        <div class="d-flex justify-center mb-30">
                            {{ $berita_dinas->links('pagination::bootstrap-4') }}
                        </div>
                    </div>
                </div>
